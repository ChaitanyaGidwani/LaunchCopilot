...
// Recent LinkedIn API versions, newest first. Override with LINKEDIN_API_VERSION.
const LINKEDIN_VERSION_FALLBACKS = ["202606", "202605", "202604", "202603", "202512"];
...
async function publishLinkedIn(app, assets) {
  ...
  // LinkedIn versions expire ~12 months after release and the API answers
  // HTTP 426 ("Upgrade Required") for anything stale. Try the configured
  // version first, then walk back through known-good recent versions so a
  // deploy never dies just because the calendar moved on.
  const versions = [
    process.env.LINKEDIN_API_VERSION,
    ...LINKEDIN_VERSION_FALLBACKS,
  ].filter(Boolean);
  ...
  for (const version of versions) {
    ...
    // 426 = unsupported API version -> try the next one. Anything else is a
    // real failure (bad token, permissions, content rejected).
    if (res.status !== 426) break;
  }

  if (!res.ok) {
    if (res.status === 426) {
      throw new Error(
        `LinkedIn rejected every API version we tried (${versions.join(", ")}). Set LINKEDIN_API_VERSION in .env.local to a currently supported YYYYMM version.`
      );
    }
    throw new Error(`LinkedIn API error ${res.status}: ${lastErr.slice(0, 200)}`);
  }
